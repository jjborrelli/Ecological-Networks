...
```{r}
motif_counter(list(SOgraph), webs = "Southern Ocean")
...
(e.g., **`r levels(s.ocean$LOCATION)[20]`**; **`r levels(s.ocean$LOCATION)[95]`**; 
**`r levels(s.ocean$LOCATION)[176]`**; etc.) included only a single predator with a portion of its prey. Others, like **`r levels(s.ocean$LOCATION)[5]`** and **`r levels(s.ocean$LOCATION)[165]`**, included a single prey with some of its predators. A few locations, however, have enough species and interactions to be conisdered a near complete food web themselves (e.g., **`r levels(s.ocean$LOCATION)[53]`**; **`r levels(s.ocean$LOCATION)[170]`**). Most webs have a small number of predators and prey and a few interactions.  
...
```{r, fig.cap = "Plot connectance agains size"}
